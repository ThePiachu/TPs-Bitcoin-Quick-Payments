...
function readAddressesFromLoginInfo(identifier, password)
{
	var xhr	= new XMLHttpRequest();
	xhr.open("GET", "https://blockchain.info/merchant/"+identifier+"/list?password="+password);
		...
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				...
				for (var i=0;i<resp["addresses"].length;i++)
				{
					...
					amount+=resp["addresses"][i]["balance"];
					if (addresses!="")
					{
						addresses+=",";
					}
					addresses+=resp["addresses"][i]["address"];
				}
